...
#
# 1. terraform apply cluster/        → EKS Auto Mode + VPC + the ACK and kro EKS
#                                        Capabilities + IAM roles (ACK capability
#                                        role, kro capability role, LiteLLM
#                                        Bedrock role + Pod Identity).
# 2. update kubeconfig
...
step "Waiting for the ACK + kro EKS Capabilities"
# ACK and kro are AWS-managed Capabilities created by terraform/cluster (step 1),
# so they are normally ACTIVE by now. ACK installs the *.services.k8s.aws
# service controllers + CRDs; kro installs the ResourceGraphDefinition CRD.
...
ok "ACK + kro Capabilities + RGDs ready"
