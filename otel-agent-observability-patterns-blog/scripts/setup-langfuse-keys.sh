...
# The password is read from the langfuse-init-keys Secret (generated by
# setup-infra.sh). Override with LANGFUSE_ADMIN_PASSWORD if needed.
...
if [ -z "$ADMIN_PASSWORD" ]; then
  ADMIN_PASSWORD=$(kubectl get secret langfuse-init-keys -n observability \
    -o jsonpath='{.data.LANGFUSE_INIT_USER_PASSWORD}' 2>/dev/null | base64 -d 2>/dev/null || echo "")
fi
if [ -z "$ADMIN_PASSWORD" ]; then
  echo "  ✗ No admin password found. Set LANGFUSE_ADMIN_PASSWORD or run setup-infra.sh first."
  exit 1
fi
...
# setup-infra.sh now generates real random keys up front and seeds Langfuse's
# project with them (headless init). So in the normal flow real keys already
# exist and this script is a no-op. It remains available as a manual fallback
# for rotating keys via the UI. "PLACEHOLDER" is the only value that means
# "not yet configured".
if [ -n "$EXISTING_PK" ] && [ "$EXISTING_PK" != "PLACEHOLDER" ]; then
